...
```cs
bool Create(string submitBy, string itemCategory, string code, string name, string specs, string desc, int attribute, int uomGroup, string uom, out string errMsg)
{
    errMsg = string.Empty;
    try
    {
        ...
        if (!string.IsNullOrEmpty(itemCategory))
        {
            ...
            {
                errMsg = $"组织编码[{org.Code}]料品分类[{itemCategory}]不存在";
                return false;
            }
        }
        ...
        if (findCostElement == null)
        {
            errMsg = $"组织编码{org.Code}找不到成本要素[材料费]";
            return false;
        }
